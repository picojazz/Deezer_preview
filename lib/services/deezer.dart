...
class Deezer {
    ...
    try {
     ...
     // print(tracksJson);
      ...
      for(var track in tracksJson){
        tracks.add(Track(track['artist']['name'],track['title'],track['preview'],track['artist']['picture_big']));
      }
      //print(tracksJson);
      return tracks;
    ...
    try {
     ...
     // print(tracksJson);
      ...
      for(var album in albumsJson){
        albums.add(Album(album['title'],album['cover_big'],album['artist']['name'],album['tracklist']));
      }
      print(albums[0]);
      return albums;
  ...
  Future getTrack(album) async {
    try {
      ...
      List<Track> tracks = new List();
      for(var track in tracksJson){
        tracks.add(Track(album.artist,track['title'],track['preview'],album.image));
      }
      /*print(tracksJson);
